  ...
  ({ className, type, variant = 'text', error, disabled, ...props }, ref) => {
    const inputElement = (
      <input
        ...
        className={cn(
          /* ================= Base ================= */
          'flex h-12 w-full rounded-lg border border-zinc-300 bg-transparent p-4 text-base placeholder:text-start text-zinc-800 transition-colors dark:border-zinc-600 dark:bg-zinc-600 dark:text-zinc-400 dark:hover:border-zinc-500 dark:focus:border-softpink-400 md:text-sm',
    ...
    // If search or tel variant, wrap with relative div for icon positioning
    if (variant === 'search' || variant === 'tel') {
      return (
    ...
    // For other variants, return input directly
    return inputElement;
  },
);
